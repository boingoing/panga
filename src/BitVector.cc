...
// We would like to use std::numeric_limits<std::byte>::max() but that isn't defined for std::byte.
...
uint32_t CountSetBits(std::byte val) {
    static constexpr int bits_set_table[256] = {0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3, 4, 1, 2, 2, 3, 2, 3, 3, 4,2, 3, 3, 4, 3, 4, 4, 5, 1, 2, 2, 3, 2, 3, 3, 4, 2, 3, 3, 4, 3, 4, 4, 5,2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6, 1, 2, 2, 3, 2, 3, 3, 4,2, 3, 3, 4, 3, 4, 4, 5, 2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6,2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6, 3, 4, 4, 5, 4, 5, 5, 6,4, 5, 5, 6, 5, 6, 6, 7, 1, 2, 2, 3, 2, 3, 3, 4, 2, 3, 3, 4, 3, 4, 4, 5,2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6, 2, 3, 3, 4, 3, 4, 4, 5,3, 4, 4, 5, 4, 5, 5, 6, 3, 4, 4, 5, 4, 5, 5, 6, 4, 5, 5, 6, 5, 6, 6, 7,2, 3, 3, 4, 3, 4, 4, 5, 3, 4, 4, 5, 4, 5, 5, 6, 3, 4, 4, 5, 4, 5, 5, 6,4, 5, 5, 6, 5, 6, 6, 7, 3, 4, 4, 5, 4, 5, 5, 6, 4, 5, 5, 6, 5, 6, 6, 7,4, 5, 5, 6, 5, 6, 6, 7, 5, 6, 6, 7, 6, 7, 7, 8};
...
BitVector::BitVector(size_t bit_count) {
    SetBitCount(bit_count);
}
...
const std::vector<std::byte>& BitVector::GetBytes() const {
    return bytes_;
}

std::vector<std::byte>& BitVector::GetBytesWritable() {
    return bytes_;
}

// static
void BitVector::WriteBytes(const std::byte* source, size_t source_start_bit_offset, std::byte* destination, size_t destination_start_bit_offset, size_t bits_to_copy) {
    const size_t destination_first_byte_index = destination_start_bit_offset / BitsPerByte;
    const size_t destination_last_byte_index = (destination_start_bit_offset + bits_to_copy) / BitsPerByte;
    ...
    if (source_start_bit_offset % BitsPerByte == 0 && destination_start_bit_offset % BitsPerByte == 0) {
        std::memcpy(destination + destination_first_byte_index, source + (source_start_bit_offset / BitsPerByte), sizeof(std::byte) * (destination_last_byte_index - destination_first_byte_index + 1));
    } else {
        // Keep track of the first byte in destination so we can put back the bits in that byte which are not supposed to be overwritten.
        ...
        std::memset(destination + destination_first_byte_index, 0, sizeof(std::byte) * (destination_last_byte_index - destination_first_byte_index + 1));

        // Now copy bits from source until we've written at least bits_to_copy.
        size_t bits_written = 0;
        while (bits_written < bits_to_copy) {
            const size_t source_byte_index = (source_start_bit_offset + bits_written) / BitsPerByte;
            const size_t source_bit_offset = (source_start_bit_offset + bits_written) % BitsPerByte;
            const size_t destination_byte_index = (destination_start_bit_offset + bits_written) / BitsPerByte;
            const size_t destination_bit_offset = (destination_start_bit_offset + bits_written) % BitsPerByte;

            // Shift the source byte such that it begins with the relevant bits and shift those so they slot into the destination byte at the correct bit offset.
            destination[destination_byte_index] |= source[source_byte_index] >> source_bit_offset << destination_bit_offset;

            // Due to the bit-shifting above, we can only ever copy the smaller of source_bit_offset and destination_bit_offset number of bits while writing a single byte.
            bits_written += std::min(BitsPerByte - source_bit_offset, BitsPerByte - destination_bit_offset);
        }
        ...
        destination[destination_first_byte_index] |= first_byte & MaxByteValue >> (BitsPerByte - destination_first_byte_bit_index);
    }
...
// static
bool BitVector::Compare(const std::byte* left, const std::byte* right, size_t bits_to_compare) {
...
void BitVector::SubVector(BitVector* destination, size_t destination_start_bit_offset, size_t source_start_bit_offset, size_t bits_to_copy) const {
    // Resize destination if it's not big enough
    destination->Resize(destination_start_bit_offset + bits_to_copy);

    WriteBytes(this->bytes_.data(), source_start_bit_offset, destination->bytes_.data(), destination_start_bit_offset, bits_to_copy);
}

size_t BitVector::HammingDistance(const BitVector& rhs) const {
    size_t distance = 0;

    // If the two BitVectors differ in the number of bits they contain, it's not clear what we should return. For now, treat this as an error condition and return infinity-ish.
...
size_t BitVector::ToStringHex(char* buffer, size_t buffer_length) const {
    ...
    if (buffer != nullptr) {
        ...
        const size_t relevant_bits = this->bit_count_ % BitsPerByte;
        const std::byte mask = MaxByteValue >> (BitsPerByte - relevant_bits);
        int bytes_written = snprintf(buffer, 3U, "%02hhx", std::to_integer<uint8_t>(this->bytes_[last_byte] & mask));
        for (size_t i = 0; i < last_byte; i++) {
            bytes_written += snprintf(buffer + 2U * (i + 1U), 3U, "%02hhx", std::to_integer<uint8_t>(this->bytes_[last_byte - i - 1U]));
        }
...
void BitVector::WriteToStreamHex(std::ostream& out) const {
    ...
    for (size_t i = 0; i < last_byte; i++) {
        out << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(this->bytes_[last_byte - i - 1U]) << std::dec;
    }
}
...
BitVector::HexFormatWrapper operator<<(std::ostream& out, const BitVector::HexFormat_t& /*manip*/) {
    return {out};
}

std::ostream& operator<<(const BitVector::HexFormatWrapper& wrapper, const BitVector& bv) {
