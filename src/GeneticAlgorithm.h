...
class GeneticAlgorithm {
 public:
  ...
  enum class MutationRateSchedule : uint8_t {
     ...
     * (self_adaptive_mutation_diversity_floor_), we will introduce more
     * aggressive mutation in order to increase diversity and keep the evolution
     * from converging.<br/> The more aggressive mutation rate is configurable
     * via self_adaptive_mutation_aggressive_rate_.<br/> When diversity is not
     * below the floor value, we use a constant mutation rate controlled by
     * mutation_rate_.
     * @see mutation_rate_
     * @see SetMutationRate
     * @see GetMutationRate
  ...
  /**
   * Get the writable genome we will use to construct Individuals which make up
   * the populations of each generation. Note: Don't change the genome while the
   ...
   */
  void SetSelfAdaptiveMutationDiversityFloor(
   ...
   */
  void SetSelfAdaptiveMutationAggressiveRate(
  ...
  /**
   * Each generation, we will construct and evaluate |population_size|
   * Individuals.
   */
  ...
  /**
   * Initialize the GeneticAlgorithm population based on
   * |initial_population|.<br/> Note: We need to call this before calling
   * Initialize().
   * @param initial_population We will construct new population members and
   * interpret these values as binary chromosome data.
   */
 ...
 protected:
  /**
   * Uses the selected crossover operator to construct |offspring| based on
   * |parent1| and |parent2|.
   * @see SetCrossoverType
   * @see CrossoverType
   */
  ...
  /**
   * Uses the selected mutation operator to mutate |individual| by
   * |mutation_percentage|.
   * @see MutatorType
   * @see SetMutatorType
   */
   ...
   * Note: May depend on the previous population being evaluated so the value
   * returned may be undefined when called while the initial population is the
   * current population. ie: This rate may only make sense during generations
   * greater than 0.
   */
  double GetCurrentMutationRate();

  /**
   * If the selector we're using requires some initialization based on the
   * population, this function will perform that initialization.
   */
  void InitializeSelector(Population* population);

  /**
   * Uses the selector to choose a pair of individuals from |population|.<br/>
   * Respects the allow_same_parent_couples_ flag to enable/disable choosing the
   * same individual for both parents.
   * @see SetAllowSameParentCouples
   * @see SelectOne
   */
  std::pair<const Individual&, const Individual&> SelectParents(
  ...
  /**
   * We store two populations and alternate between them between generations. In
   * this way, the previous and current generations are always available.<br/>
   * This function will always return the current population.<br/>
   * The current population is the one which is being built, evaluated, and
   * scored during the current generation of the genetic algorithm.
   * @see populations_
   ...
   * We store two populations and alternate between them between generations. In
   * this way, the previous and current generations are always available.<br/>
   * This function will always return the population from the previous
   * generation.<br/> The previous generation population is the one which was
   * built, evaluated, and scored during the previous generation of the genetic
   * algorithm.
   * @see populations_
 ...
 private:
  ...
  double self_adaptive_mutation_diversity_floor_ =
      DefaultSelfAdaptiveMutationDiversityFloor;
  double self_adaptive_mutation_aggressive_rate_ =
      DefaultSelfAdaptiveMutationAggressiveRate;
