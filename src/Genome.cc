...
size_t Genome::AddGene(size_t bit_width, bool byte_align) {
    ...
    if (byte_align) {
        // If the gene would start after a byte border, adjust the start index forward to the next byte.
        ...
        // If the gene would end in the middle of a byte, adjust the width such that it will end at a byte border instead.
    ...
    // Update the bit index for the start of the boolean genes to point just after the new gene we added.
