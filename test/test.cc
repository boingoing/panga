...
double TestObjective(Individual* individual, void* user_test_data) {
  ...
  if (VerboseOutput) {
    std::cout << "Individual " << test_data->current_generation << ":"
              << test_data->current_individual << " => Error " << fail_bits
              << std::endl;
  }
...
bool TestSolveMatchingProblem(const char* target) {
  ...
  AssertTrue(genome.BitsRequired() == test_data.target_bits.GetBitCount(),
             "Genome encodes correct number of bits");
  ...
  ga.SetMutationRateSchedule(
      GeneticAlgorithm::MutationRateSchedule::Proportional);
...
bool TestCrossoverGenes(size_t gene_count, size_t gene_width) {
  ...
  AssertTrue(genome.BitsRequired() == gene_width * gene_count,
             "Genes add up to the corect length");

  Individual left(genome);
  Individual right(genome);
  Individual offspring(genome);
  AssertTrue(genome.BitsRequired() == left.GetBitCount(),
             "Individuals based on a genome have length eqaul to the genome "
             "bits required");

  // Alternate gene values between all 0s and all 1s
  for (size_t i = 0; i < gene_count; i++) {
    const uint64_t left_value =
        i % 2U == 0 ? std::numeric_limits<uint64_t>::max() : 0;
    const uint64_t right_value =
        i % 2U == 0 ? 0 : std::numeric_limits<uint64_t>::max();
  ...
  for (size_t i = 0; i < gene_count; i++) {
    ...
    const auto max_value = std::numeric_limits<uint64_t>::max() >>
                           (sizeof(uint64_t) * CHAR_BIT - gene_width);
    AssertTrue(value == 0 || value == max_value,
               "Decoded value should be all 1s or all 0s");
  }

  return true;
}

bool TestBitVectorToString(BitVector* bv, const char* expectedBinString,
                           const char* expectedHexString) {
  constexpr size_t buf_size = 2000;
  char buf[buf_size];
  bv->ToStringHex(buf, sizeof(buf));
  AssertTrue(std::strcmp(buf, expectedHexString) == 0,
             "BitVector::ToStringHex produces expected result");
  std::stringstream sstr1;
  sstr1 << BitVector::HexFormat << *bv;
  AssertTrue(std::strcmp(buf, sstr1.str().c_str()) == 0,
             "BitVector::ToStringHex produces same result as operator<<");

  bv->ToString(buf, sizeof(buf));
  AssertTrue(std::strcmp(buf, expectedBinString) == 0,
             "BitVector::ToString produces expected result");
  std::stringstream sstr2;
  sstr2 << *bv;
  AssertTrue(std::strcmp(buf, sstr2.str().c_str()) == 0,
             "BitVector::ToString produces same result as operator<<");

  BitVector tbv;
  tbv.FromString(expectedBinString, strlen(expectedBinString));
  AssertTrue(bv->Equals(tbv),
             "BitVector::FromString produces correct BitVector");

  tbv.FromStringHex(expectedHexString, strlen(expectedHexString));
  AssertTrue(bv->Equals(tbv),
             "BitVector::FromStringHex produces correct BitVector");
...
const BitVectorTest BitVectorTests[] = {
    ...
    {20,
     {{0xff, 0, 8}, {0xff, 18, 2}, {524288, 0, 20}},
     {},
     "10000000000000000000",
     "080000"},
    {20,
     {{0xff, 0, 8}, {0xff, 18, 2}, {524288, 0, 20}, {0xff, 4, 8}},
     {},
     "10000000111111110000",
     "080ff0"},
    {20, {}, {16}, "00010000000000000000", "010000"},
    {100,
     {{0xfff, 1, 16}, {0xfff, 81, 16}},
     {},
     "0000000111111111111000000000000000000000000000000000000000000000000000000"
     "000000000000001111111111110",
     "001ffe00000000000000001ffe"},
};
